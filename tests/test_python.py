...
def test_model_forward():
    ...
    model = YOLO(CFG)
    model(source=None, imgsz=32, augment=True)  # also demo no source and augment
...
def test_youtube():
    ...
    Note: ConnectionError may occur during this demo due to network instability or YouTube server availability.
    """
    model = YOLO(MODEL)
    try:
...
def test_track_stream():
    ...
    model.track(video_url, imgsz=160, tracker="botsort.yaml", save_frames=True)  # demo frame saving also
...
def test_utils_patches_torch_save():
    ...
    with patch("ultralytics.utils.patches._torch_save", new=mock):
        with pytest.raises(RuntimeError):
            torch_save(torch.zeros(1), TMP / "demo.pt")
...
def test_model_embeddings():
    ...
    for batch in [SOURCE], [SOURCE, SOURCE]:  # demo batch size 1 and 2
...
def test_yolo_world():
    ...
    # demo WorWorldTrainerFromScratch
...
def test_yolov10():
    """A simple demo for yolov10 for now."""
